...
def make_ring_icp_factor(key1, key2,
                         points1: np.ndarray,
                         points2: np.ndarray,
                         normals2: np.ndarray,
                         noise_sigma: float) -> gtsam.CustomFactor:
    """
    ...
    def error_func(this: gtsam.CustomFactor,
        ...
        # Get the same keys back (we already know them, but this is generic)
        k1 = this.keys()[0]
        k2 = this.keys()[1]
        ...
        # --- Jacobians ---
        # For now: fill with zeros so dimensions are correct.
        # (Factor won't move poses yet, but no crash.)
        if H is not None:
            # one Jacobian per key: Pose3 -> 6 DOF
            # H[0]: d r / d pose1, H[1]: d r / d pose2
            H[0] = np.zeros((N, 6))
            H[1] = np.zeros((N, 6))
...
class ImuPreintegrationNode(Node):
    def __init__(self):
        ...
        self.pub_laser_topic = self.get_parameter('pub_laser_topic').get_parameter_value().string_value
    ...
    def laser_callback(self, msg: PointCloud2):
            ...
            if pts.shape[0] < 10:
                return  # skip empty or tiny clouds

            # ---- 2. Estimate normals (simple PCA per point or global circle normal) ----
            # For a ring, normals all point OUTWARD from center
            # Estimate center:
            ...
            # ---- 3. Store ring data for current keyframe index ----
            k = self.frame_idx
