...
# -*- coding: utf-8 -*-
"""
IMU Preintegration + Laser Profiler ring ICP (GTSAM) + corrected cloud in odom frame.

Based on imu_topic_laser.py, extended to:
 - use base_link -> laser_profiler_link extrinsic from Gazebo
 - publish optimized laser profiler point cloud in odom frame
"""
...
def make_ring_icp_factor(
    ...
    def error_func(this: gtsam.CustomFactor,
        ...
        # Jacobians: for now just zeros with correct shape to avoid dimension errors.
        if H is not None:
            H[0] = np.zeros((N, 6))
            H[1] = np.zeros((N, 6))
...
class ImuPreintegrationNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('pub_odom_topic', '/imu_preintegration_with_laser/odom')
        self.declare_parameter('pub_path_topic', '/imu_preintegration_with_laser/path')
        self.declare_parameter('pub_laser_topic', '/inpipe_bot/laser_profiler/points')
        ...
        # 🔧 body_T_lidar: use Gazebo joint base_link -> laser_profiler_link:
        # <origin xyz="0.3 0.06 0" rpy="0 1.5708 0"/>
        # which is (x=0.3, y=0.06, z=0, roll=0°, pitch=90°, yaw=0°)
#        self.declare_parameter('body_T_lidar', [0.3, 0.06, 0.0, 0.0, 90.0, 0.0])
        ...
        # Calibration / stationary detection (like example node)
        ...
        self.pub_laser_topic = self.get_parameter('pub_laser_topic').value
        ...
        # Extrinsic body -> lidar (IMU/body = base_link here)
        extr = [float(x) for x in self.get_parameter('body_T_lidar').value]
        # extr = [x, y, z, roll_deg, pitch_deg, yaw_deg]
        ...
        # Ring storage per keyframe index
        self.ring_data = {}
        ...
        # Publisher for corrected cloud in odom frame
        ...
        self.get_logger().info(
            'IMU Preintegration node initialized (preintegration logic mirrors imu_topic_example.py).'
        )
    ...
    def laser_callback(self, msg: PointCloud2):
        ...
        Compute normals and store as ring_data[current_frame_idx].
        ...
        self.get_logger().info(
            f"Stored laser ring for frame {k}, {pts.shape[0]} points"
        )
    ...
    def lidar_callback(self, odom_msg: Odometry):
        ...
        # 🔥 Publish corrected laser cloud in odom frame
...
#        pose_k_body = pose_k_lidar.compose(self.body_T_lidar.inverse())
        ...
        if (k-1 in self.ring_data) and (k in self.ring_data):
            ...
            if N > 0:
                ...
                self.get_logger().info(
                    f"Added ring ICP factor between frame {k-1} and {k} with {N} points"
                )
        else:
    ...
    def publish_outputs(self, odom_src: Odometry, pose: Pose3, vel):
          ...
          try:
            ...
            # If t is a numpy array-like
            tx, ty, tz = float(t[0]), float(t[1]), float(t[2])

          # --- robust rotation extraction (handles Rot3 versions) ---
          R = pose.rotation()
          try:
            # Some bindings expose quaternion() -> [w, x, y, z]
            qw, qx, qy, qz = map(float, R.quaternion())
          except Exception:
            # Others use toQuaternion() returning a Quaternion object
            q = R.toQuaternion()
    ...
    def publish_corrected_cloud(self, pose: Pose3, stamp):
          ...
          R = T_odom_laser.rotation().matrix()
          ...
          P_odom = R @ P_laser + t_vec  # 3 x N
          points_odom_list = P_odom.T.tolist()  # N x 3

          # Build output cloud in odom frame
          header = msg.header
          header.stamp = stamp
          header.frame_id = 'odom'  # or 'pipe_odom' if that's your odom frame
