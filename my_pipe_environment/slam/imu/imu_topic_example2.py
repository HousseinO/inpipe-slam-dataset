...
from gtsam import (
    ...
    PriorFactorPose3, PriorFactorVector, PriorFactorConstantBias, BetweenFactorConstantBias, ImuFactor,
...
class ImuPreintegrationNode(Node):
    def __init__(self):
        ...
        # Lidar odom fusion noise (Pose prior at each LiDAR frame)
    ...
    def add_sequential_factors(self, odom_msg: Odometry):
        ...
        # 3) LiDAR odometry as a Pose3 prior-like factor at time k
        pose_k = self.pose_from_odom(odom_msg)
        self.graph.add(PriorFactorPose3(X(k), pose_k, self.lidar_pose_noise))
