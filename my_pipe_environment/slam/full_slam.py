...
IMU Preintegration + dual-mode SLAM:
 - Pipe mode:   IMU + /pipe/odom + ring ICP (laser profiler)
 - Elbow mode:  IMU + /odom/rplidar (2D lidar odom), locally anchored
                at the optimized pose at the end of the pipe.

Mode switch driven by /inpipe_bot/rplidar/points:
 - Detect first elbow when any point is within elbow_distance_threshold
   in front or behind the 2D lidar.
 - On first elbow detection:
     * Store current optimized pose from pipe graph as elbow_anchor_pose
     * Stop using /pipe/odom
     * Start elbow graph from elbow_anchor_pose and use /odom/rplidar only
       as relative odometry (deltas between consecutive poses).
"""
...
class FullSlamNode(Node):
    def __init__(self):
        ...
        # Two separate ISAM2 instances: pipe-mode and elbow-mode
        self.isam_pipe = ISAM2(isam_params)
        self.isam_elbow = ISAM2(isam_params)
        ...
        # Elbow anchor state
        self.first_elbow_reached = False
        self.elbow_anchor_pose = None   # will be Pose3 when set
        ...
        self.get_logger().info(
            'Full SLAM node initialized: pipe-mode + anchored elbow-mode using /odom/rplidar.'
        )
    ...
    def rplidar_points_callback(self, msg: PointCloud2):
        ...
        in front or behind the lidar. If yes -> near_elbow = True.

        On the *first* transition False -> True:
          - mark first_elbow_reached = True
          - elbow_anchor_pose = current optimized pose (end of pipe)
          - reset elbow graph to start from that anchor.
        """
        min_r = None
        ...
        # First time we enter elbow region
        if self.near_elbow and not prev_near and not self.first_elbow_reached:
            self.first_elbow_reached = True
            # Anchor elbow pose at current optimized pose from pipe graph
            try:
                self.elbow_anchor_pose = self.prev_state.pose()
            except Exception:
                self.elbow_anchor_pose = Pose3()

            # Reset elbow graph so it starts from anchor
            self.frame_idx_elbow = 0
            self.graph_elbow = NonlinearFactorGraph()
            self.initial_elbow = Values()
            # Reset last lidar pose for elbow graph
            if hasattr(self, 'last_lidar_body_pose_elbow'):
                del self.last_lidar_body_pose_elbow

            self.get_logger().info(
                f"[rplidar_points_callback] First elbow detected (min_r={min_r:.2f} m). "
                f"Anchoring elbow graph at current SLAM pose."
            )

    def pipe_odom_callback(self, odom_msg: Odometry):
        """
        In-pipe SLAM: IMU + /pipe/odom + ring ICP.
        Used only *before* the first elbow has been reached.
        After first_elbow_reached, /pipe/odom is ignored.
        """
        if self.first_elbow_reached:
            # We consider pipe finished; ignore further /pipe/odom
            self.get_logger().debug("[pipe_odom_callback] first_elbow_reached=True -> ignoring /pipe/odom")
            return
    ...
    def rplidar_odom_callback(self, odom_msg: Odometry):
        ...
        Only used when:
          - first_elbow_reached == True (we already anchored elbow graph)
          - elbow_anchor_pose is set
          - near_elbow == True (we are in elbow region)
        """
        if not self.first_elbow_reached or self.elbow_anchor_pose is None:
            self.get_logger().debug(
                "[rplidar_odom_callback] first_elbow_reached=False or anchor unset "
                "-> ignoring /odom/rplidar"
            )
            return

        if not self.near_elbow:
            self.get_logger().debug(
                "[rplidar_odom_callback] near_elbow=False -> ignoring /odom/rplidar"
            )
            return
    ...
    def add_first_frame_factors_elbow(self, odom_msg: Odometry):
        ...
        Prior pose is:
         - elbow_anchor_pose (optimized end-of-pipe pose) if available
         - otherwise falls back to pose_from_odom(odom_msg)
        """
        if self.elbow_anchor_pose is not None:
            pose0 = self.elbow_anchor_pose
        else:
            pose0 = self.pose_from_odom(odom_msg)
    ...
    def publish_outputs(self, odom_src: Odometry, pose: Pose3, vel):
        ...
        # --- robust translation extraction (handles Point3 or numpy array) ---
        t = pose.translation()
        try:
        ...
        # --- robust rotation extraction (handles Rot3 versions) ---
        R = pose.rotation()
        try:
