...
"""
Full SLAM node (rewritten, without ring laser profiler):

- PIPE mode:
    IMU + /pipe/odom   (no ring ICP)
...
- Repeating sequence:
    PIPE -> ELBOW -> PIPE -> ELBOW -> PIPE -> ...

- When exiting an elbow:
    The next PIPE segment is re-initialized with a prior at the
    last optimized elbow pose (no jump), and a fresh pipe graph.
"""
...
class FullSlamNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('lidar_pose_sigma_rpy_deg', 20.0)
        ...
        # Frame indices
        self.frame_idx_pipe = 0   # used for pipe graph
        ...
        # 2D lidar (RPLidar) – points + odom for elbow detection and ELBOW mode
        ...
        self.get_logger().info(
            'Full SLAM node initialized (PIPE: IMU + /pipe/odom, '
            'ELBOW: IMU + /odom/rplidar), no ring laser profiler.'
        )
    ...
    def add_sequential_factors_pipe(self, odom_msg: Odometry):
        ...
        if self.last_dt > 1e-6:
            ...
            # If no IMU integration, softly keep velocity close to previous
            self.graph_pipe.add(
