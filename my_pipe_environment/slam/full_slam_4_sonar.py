...
"""
Full SLAM node (PIPE/ELBOW) with sonar blended into pipe odometry:

- PIPE mode:
    IMU + /pipe/odom (3D) + ring ICP (laser profiler)
    + sonar axial fusion into /pipe/odom delta.

- ELBOW mode:
    IMU + /odom/rplidar (2D lidar odom), locally anchored
    at the optimized pose when entering the elbow.

Sonar fusion (Option B, no landmark):
    Let s_k = d_{k-1} - d_k (sonar distance change)
    Let dx_pipe = axial translation from /pipe/odom between k-1,k

    dx_fused = dx_pipe + alpha * (s_k - dx_pipe)

    alpha in [0,1] controls how much sonar corrects axial motion.

We then use dx_fused as the X-translation of the BetweenFactorPose3
between X(k-1) and X(k) in PIPE mode.
"""
...
def make_ring_icp_factor(
    ...
    def error_func(this: gtsam.CustomFactor,
        ...
        # Residuals r_i = n_i^T (p2_i - (R p1_i + t))  → shape (N,)
        residuals = np.sum(N2 * diff, axis=0)

        # IMPORTANT: give GTSAM Jacobians with the correct shape (N x 6)
        if H is not None:
            N = residuals.shape[0]
            H[0] = np.zeros((N, 6), dtype=float)
            H[1] = np.zeros((N, 6), dtype=float)
...
class FullSlamNode(Node):
    def __init__(self):
        ...
        # Sonar topic + noise + blending
        self.declare_parameter('sonar_topic', '/inpipe_bot/sonar/right/max_range')
        self.declare_parameter('sonar_max_valid', 15.0)     # [m] ignore if > this (no hit)
        self.declare_parameter('sonar_min_valid', 4.0)
        self.declare_parameter('sonar_alpha', 1.0)         # fusion weight alpha 0.7
        ...
        # Priors
        self.declare_parameter('prior_pose_sigma_xyz', 0.1)
        self.declare_parameter('prior_pose_sigma_rpy_deg', 180.0)
        ...
        self.sonar_max_valid = float(self.get_parameter('sonar_max_valid').value)
        self.sonar_min_valid = float(self.get_parameter('sonar_min_valid').value)
        self.sonar_alpha = float(self.get_parameter('sonar_alpha').value)
        ...
        # Latest sonar range (Float32)
        self.latest_sonar_range = None
        # Sonar distance per PIPE frame index
        self.sonar_range_pipe = {}

        # Sonar beam direction in base_link frame.
        # Assume joint rpy="0 0 0": sonar +X == base_link +X.
        self.sonar_beam_dir_body = np.array([1.0, 0.0, 0.0], dtype=float)
        ...
        # Where the next pipe segment should be anchored (global Pose3)
        self.pipe_anchor_pose = None

        # Mode machine (PIPE / ELBOW) and elbow counter
        self.mode = "PIPE"
        self.elbow_count = 0
        ...
        self.path_msg = Path()
        self.path_msg.header.frame_id = 'odom_slam'
        ...
        self.get_logger().info(
            'Full SLAM node initialized with PIPE/ELBOW modes, '
            'ring ICP, and sonar-blended pipe odometry in PIPE segments.'
        )
    ...
    def sonar_callback(self, msg: Float32):
        """
        Store latest sonar max distance (Float32).
        It will be associated to a frame index inside pipe_odom_callback.
        """
        d = float(msg.data)
        if d<= self.sonar_min_valid:
            self.sonar_alpha = 0.0 
        elif d>self.sonar_min_valid:
            self.sonar_alpha = 1.0
        
        if not math.isfinite(d):
            return

        if d <= self.sonar_min_valid or d > self.sonar_max_valid:
            self.sonar_alpha = 0.0
            return
        

        self.latest_sonar_range = d
    ...
    def rplidar_points_callback(self, msg: PointCloud2):
        ...
        if self.near_elbow and not prev_near and self.mode == "PIPE":
            ...
            # Reset IMU integration boundary for elbow
            self.last_lidar_time = None
        ...
        if (not self.near_elbow) and prev_near and self.mode == "ELBOW":
            self.get_logger().info(
                ...
                f"switching back to PIPE mode and anchoring next pipe segment."
            )

            # 1) Anchor next pipe segment at the current optimized pose (end of elbow)
            try:
                self.pipe_anchor_pose = self.prev_state.pose()
            except Exception:
                self.pipe_anchor_pose = Pose3()

            # 2) Reset PIPE graph and index so the new pipe section starts fresh
            self.frame_idx_pipe = 0
            self.graph_pipe = NonlinearFactorGraph()
            self.initial_pipe = Values()
            self.isam_pipe = ISAM2(self.isam_params)

            if hasattr(self, 'last_lidar_body_pose_pipe'):
                del self.last_lidar_body_pose_pipe

            # Reset sonar buffer for new pipe segment
            self.sonar_range_pipe.clear()

            # Reset IMU integration boundary for new pipe segment
            self.last_lidar_time = None
    ...
    def pipe_odom_callback(self, odom_msg: Odometry):
        """
        PIPE SLAM: IMU + /pipe/odom + ring ICP + sonar axial fusion.
        ...
        # Attach latest sonar range to this PIPE frame index (if available)
        if self.latest_sonar_range is not None:
            self.sonar_range_pipe[self.frame_idx_pipe] = self.latest_sonar_range
    ...
    def add_first_frame_factors_pipe(self, odom_msg: Odometry):
        """
        First node of a PIPE segment.

        If pipe_anchor_pose is set (e.g. after elbow exit), use it as prior.
        Otherwise, use pose_from_odom(odom_msg) for the very first segment.
        """
        if self.pipe_anchor_pose is not None:
            pose0 = self.pipe_anchor_pose
            self.get_logger().info(
                "[add_first_frame_factors_pipe] Using pipe_anchor_pose as prior for new pipe segment."
            )
        else:
            pose0 = self.pose_from_odom(odom_msg)
            self.get_logger().info(
                "[add_first_frame_factors_pipe] Using /pipe/odom pose as prior for first pipe segment."
            )
        ...
        # Anchor used; clear it so future resets must set it explicitly
        self.pipe_anchor_pose = None
    ...
    def add_sequential_factors_pipe(self, odom_msg: Odometry):
        ...
        # --- Sonar-based axial fusion: dx_fused = dx_pipe + alpha (s - dx_pipe) ---
        if (k - 1 in self.sonar_range_pipe) and (k in self.sonar_range_pipe) and self.sonar_alpha!=0.0:
            d_prev = self.sonar_range_pipe[k - 1]
            d_curr = self.sonar_range_pipe[k]
            s = d_prev - d_curr        # expected axial step (sonar)
            ...
            dx_pipe = tx
            alpha = self.sonar_alpha
            print('alpha = ' , alpha)
            dx_fused = dx_pipe + alpha * (s - dx_pipe)
            tx = dx_fused
            ...
            self.get_logger().info(
                f"[SONAR-ODO] k={k}: dx_pipe={dx_pipe:.3f}, s={s:.3f}, "
                f"dx_fused={dx_fused:.3f}, d_prev={d_prev:.3f}, d_curr={d_curr:.3f}"
            )
        ...
        if (k-1 in self.ring_data) and (k in self.ring_data):
            ...
            if N > 0:
                ...
                self.get_logger().info(
                    f"Added ring ICP factor between pipe frames {k-1} and {k} "
                    f"with {N} points"
                )
        else:
            self.get_logger().debug(
    ...
    def publish_outputs(self, odom_src: Odometry, pose: Pose3, vel):
        ...
        odom_out.child_frame_id = 'base_link_slam'
        ...
        tf_msg.header.frame_id = 'odom_slam'
        tf_msg.child_frame_id = odom_out.child_frame_id  # typically 'base_link'
